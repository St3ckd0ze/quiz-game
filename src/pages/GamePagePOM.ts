...
export default class GamePagePOM extends AbstractPOM {
    ...
    private maxRounds = 3; // Erhöht für Ergebnisseite
    private questions: any[] = [];
    private categories: string[] = [];

    private async loadQuestions(round: number) {
        const res = await fetch(`/questions_round${round}.json`);
        this.questions = await res.json();
        this.categories = [...new Set(this.questions.map((q: any) => q.category))];
    ...
    public async loadPage(): Promise<void> {
        ...
        this.renderBoard();
        (window as any).GamePagePOMInstance = this;
    }
    ...
    public checkAllHidden() {
        ...
        const allHidden = Array.from(cells).every(cell => (cell as HTMLElement).dataset.state === 'hidden');
        const nextRoundBtn = document.getElementById('next-round-btn');
        const resultsBtn = document.getElementById('results-btn');
        
        if (allHidden) {
            if (this.currentRound === 1 && this.currentRound < this.maxRounds) {
                if (!nextRoundBtn) this.showNextRoundButton();
            } else if (this.currentRound === 2) {
                if (!resultsBtn) this.showResultsButton();
            }
        } else {
    ...
    private async showResultsButton() {
        const btn = document.createElement('button');
        btn.id = 'results-btn';
    ...
    private async loadAndDisplayResults() {
        try {
            ...
            const podiumPlaces: { place: number, points: number, teams: any[] }[] = [];
            ...
            for (const team of teams) {
                if (currentPoints === null || team.points !== currentPoints) {
                    ...
                    if (podiumPlaces.length === 3) break;
